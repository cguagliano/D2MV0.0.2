...
public class myLwPolyline extends myEntity{
	...
	public myLwPolyline(String name, int flag, int c, myLayer l, Vector<myLwVertex> v, int visibility, myLineType lineType, double thickness){
	...
	public myLwPolyline(myLwPolyline orig) {
		...
		for (int i=0;i<orig._myVertex.size();i++) {
			_myVertex.add(new myLwVertex((myLwVertex)orig._myVertex.elementAt(i),true));
		}
		_flag		= orig._flag;
	}
	...
	public static myLwPolyline read(myBufferedReader br, myUnivers univers) throws IOException {
...
//		myLwPolyline 			p		= null;	
		...
		return new myLwPolyline(name, flag, color, l, lv, visibility, lineType, myTable.defaultThickness);
	}
